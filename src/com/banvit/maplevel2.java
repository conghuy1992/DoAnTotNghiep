...
public class maplevel2 extends BaseGameActivity implements
		IOnSceneTouchListener {
	String prefname = "my_data2";
	BoundCamera _camera;
	...
	Texture _Texture, _Texture2, _Texture3, _Texture4, _Texture5, _Texture6,
			...
			_Texture12, bonus, bonus3, target6, bird, _texture14, _texture13;
	TextureRegion _Region, _Region3, _Region5, _Region7, _Region8, _Region9,
			_Region11, _Region10, _Region12, Rbonus, Rbonus3, Rtarget6,_region14,_region13;
	...
	long _diem = 0, _loi = 0, deadline = 350;
	...
	public Engine onLoadEngine() {
		...
		try {
			   if (MultiTouch.isSupported(this)) {
			    _engine.setTouchController(new MultiTouchController());
			    if (MultiTouch.isSupportedDistinct(this)) {
			     //Toast.makeText(this, "Cáº£m á»©ng Ä‘a Ä‘iá»ƒm - cÃ¡c Ä‘iá»�?u khiá»ƒn hoáº¡t Ä‘á»™ng tá»‘t!", Toast.LENGTH_LONG).show();
			    } else {
			     Toast.makeText(this, "MultiTouch detected, but your device has problems " +
			   ...
			   Toast.makeText(this, "Báº£n android nÃ y khÃ´ng há»— trá»£ Ä‘a Ä‘iá»ƒm!\n\n(Chá»‰ sá»­ dá»¥ng Ä‘Æ¡n Ä‘iá»ƒm.)\n\nChá»‰ sá»­ " +
			     "dá»¥ng Ä‘Æ°á»£c má»™t Ä‘iá»�?u khiá»ƒu trong má»™t thá»�?i Ä‘iá»ƒm.", Toast.LENGTH_LONG).show();
			  }
		return _engine;
	}
	...
	public void onLoadResources() {
		...
		//
		//
		_Texture4 = new Texture(1024, 256,
				TextureOptions.BILINEAR_PREMULTIPLYALPHA);
		...
		//
		_Texture5 = new Texture(2048, 512,
				TextureOptions.BILINEAR_PREMULTIPLYALPHA);
		...
		//
		_Texture7 = new Texture(256, 256,
				TextureOptions.BILINEAR_PREMULTIPLYALPHA);
		_Region7 = TextureRegionFactory.createFromAsset(_Texture7, this,
				"danTarget.png", 0, 0);
		mEngine.getTextureManager().loadTexture(_Texture7);
		//
		...
		//
		_Texture8 = new Texture(1024, 128,
				TextureOptions.BILINEAR_PREMULTIPLYALPHA);
		...
		_winGame = new Sprite(xwin, ywin, _Region11) {
			...
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent,
				...
				if (pSceneTouchEvent.isActionDown()) {
					...
					this.setVisible(false);
					Intent intent = new Intent(maplevel2.this, map3.class);
	...
	// dan target
	public void _adtarget3() {

		Random rand = new Random();

		int x = (int) _camera.getWidth() + _Region7.getWidth();
		int minY = _Region7.getHeight();
		int maxY = (int) (_camera.getHeight() - _Region7.getHeight());
		int rangeY = maxY - minY;
		int y = rand.nextInt(rangeY) + minY;

		Sprite _dantg = new Sprite(x, y, _Region7);
		_dantg.registerEntityModifier(new SequenceEntityModifier(
				new ParallelEntityModifier(new AlphaModifier(13, 0, 1),
						new ScaleModifier(19, 0.5f, 1), new RotationModifier(
								21, 0, 360))));

		_scene.attachChild(_dantg);
		int ymin = 2;
		int ymax = (int) _WIDTH - _Region7.getHeight();
		int yy = ymax - ymin;
		int ymod = rand.nextInt(yy) + ymin;
		MoveModifier mod = new MoveModifier(23, _dantg.getX(),
				-_dantg.getWidth(), _dantg.getY(), ymod);
		_dantg.registerEntityModifier(mod);

		addTarget3.add(_dantg);
	...
	//
	public void createtargetLL3() {
		TimerHandler spriteTimerHandler;
		float Thoigianlap = 5f;// th�?i gian lặp của shoot target
...
//long rung
			...
			while (targets6.hasNext()) {
				...
				if (_target6.getX() <= -_target6.getWidth()) {
					removeSprite6(_target6, targets6);
					break;
				}
				//
				Iterator<Sprite> projectiles = dan.iterator();
...
//		int rangex = maxx - minx;
	...
	public void removeSprite6(final Sprite _sprite, Iterator<Sprite> it) {
		runOnUpdateThread(new Runnable() {
	...
	public void restoringPreferences() {
		SharedPreferences pre = getSharedPreferences(prefname, MODE_PRIVATE);
		long best = pre.getLong("highscore", 350);
		long i = 350 - best;
